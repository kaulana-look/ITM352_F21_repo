...
//Callback function to check whether array elements are non-negative integerss
function checkIt(elem, index) {
    console.log(`${index}: ${elem} is ${(isNonNegativeInteger(elem) ? 'a' : 'not a')} valid quantity`); + elem + " " + isNonNegativeInteger(elem));
}
...
pieces.forEach( (elem, index) => {console.log(`${index}: ${elem} is ${(isNonNegativeInteger(elem) ? 'a' : 'not a')} valid quantity`); + elem + " " + isNonNegativeInteger(elem));}